...
def search(empid,empname,startdate,enddate,ordercap,ordtxt,regexp,state,listnum):
    try:
        ...
        DF.rename(columns={DF.columns[0]:"empid",DF.columns[1]:"empname",DF.columns[2]:"mn"},inplace=True)
        ...
        if empidreshima or empname == "רשימה":
            ...
            empid = DF.loc[DF["empname"].str.contains(r'.*{}.*'.format(empname),na=False,regex=True),"empid"].unique().tolist()
            empid = tuple(empid)
        #
        elif empid == "" and empname == "":
            empid = ""
